...
int main() {
    ...
    while (t--){
        ...
        for (edge &e: edges){
            ...
            e.x--, e.y--;
            if (e.type == 1) {
                //cout << "yes " << e.x << " " << e.y << endl;
                adjList[e.x].push_back(e.y);
            }
        }

        vii order = topological_sort(adjList);
        ...
        cout << "YES" << endl;
        vii idx(n, -1);
        f(i, 0, n) idx[order[i]] = i;
        //for (auto e : idx) cout << e << " "; cout << endl;
        for (edge &e: edges){
            if (idx[e.x] > idx[e.y]) swap(e.x, e.y);
            cout << ++e.x << " " << ++e.y << endl;
        }
    }
}