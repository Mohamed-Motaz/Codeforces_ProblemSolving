...
int main() {
    ...
    for (auto e: s) if ((e - '0') % 2 == 0){
        even = true;
        break;
    }

    if (!even) return cout << -1, 0;

    //search for first even num smaller than me from the left
    //if (!found) search for first even num from the right
