        ...
        f(i, 0, s.size() + 1) {
            ...
            if (i == s.size()) {
                if (s[i - 1] == '0') {
                    ...
                    v.push_back({{counter, 0}, {first, last}});
                }
                break;
            }
            if (s[i] == '0' && !done) {
            ...
            for (auto e: v) {
                ...
                //cout << e.first.first << " " << e.second.first << " " << e.second.second << endl;
                ans = 0;
                if (e.second.first && e.second.second ) 0;
                else if (e.second.first || e.second.second){
                    e.first.first -= m;
                }
                else e.first.first -= 2 * m;
                while (e.first.first>0){
                    ans += 1;
                    e.first.first -= (m+1);
                }
                t += ans;
