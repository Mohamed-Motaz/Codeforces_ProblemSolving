...
ll solve(int curIdx, int prevState){
    if (curIdx >= n + 1) {
        can = true; //reached the end
        return 0;
    }
    ll &ans = dp[curIdx][prevState];
    if (ans != -1) return ans;
    ans = llinf;
    string t = strings[curIdx];
    reverse(all(t));
    string prev = strings[curIdx - 1];
    if (prevState == 1) reverse(all(prev));
    bool in = false;
    if (!lexicographical_compare(all(strings[curIdx]), all(prev))){
        in = true;
        ll op = solve(curIdx + 1, 0);
        ans = min(ans, op);
    }
    if (!lexicographical_compare(all(t), all(prev))){
        in = true;
        ll op = cost[curIdx] + solve(curIdx + 1, 1);
        ans = min(ans, op);
    }
    if (!in){
        return llinf;  //failed to go into any one
    }
...
int main() {
    MohamedMotaz;
    //string t ="abz", tt = "aca";
    //cout << lexicographical_compare(all(t),all(tt));
