...
ll solve(ll index, ll edges){
    if (edges == 0){
        if (index == n) return 0;
        return inf;
    }
    if (dp[index][edges] != -1) return dp[index][edges];
    ll ans = inf;
    for (auto child: adjList[index]) ans = min(ans, solve(child.first, edges - 1) + child.second);
    return dp[index][edges] = ans;
}

void buildSol(ll start, ll end){
    cout << start << " ";
    if (end == 0 && start == n) return;
    for (auto elem: adjList[start]){
        if (dp[start][end] == solve(elem.first, end - 1) + elem.second){
            buildSol(elem.first, end - 1); return;
        }
    }
}
