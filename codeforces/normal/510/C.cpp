...
int main() {
    ...
    f(i, 0, n - 1){
        ...
        if (idx == -1){
            if (v[i].size() > v[i + 1].size()) return cout << "Impossible", 0;
        }
        else adjList[v[i + 1][idx] - 'a'].push_back(v[i][idx] - 'a');
    }
    ...
    if (oops) return cout << "Impossible", 0;
    vector<int> ans = topological_sort(adjList);
    for (auto e: ans) {
        char tmp = e + 'a';
        if (tmp <= 'z' && tmp >= 'a') cout << tmp;
        //cout << (char)(e + 'a');
    }
