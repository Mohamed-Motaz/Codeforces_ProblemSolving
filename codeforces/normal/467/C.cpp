...
vll cumSum;
ll dp[5007][5007];
ll solve(int idx, int ctr){
    if (idx > n && ctr == k) return 0;
    if (idx > n && ctr < k) return -1e12;

    ll &ans = dp[idx][ctr];
    if (ans != -1) return ans;
    ans = solve(idx + 1, ctr);
    if (idx + m  - 1 <=n  && ctr + 1 <= k) ans = max(ans,
            (cumSum[idx + m - 1] - cumSum[idx - 1]) + solve(idx + m, ctr + 1));
    return ans;
}
int main() {
    MohamedMotaz;
    ...
    f(i, 1, n + 1) cin >> v[i];
    cumSum[1] = v[1];
    f(i, 2, v.size()) cumSum[i] += cumSum[i - 1] + v[i];
