...
int main() {
	...
	while (t--) {
		...
		if (abs(x2 - x1) == 0 && abs(y2 - y1) == 0 && (a || b || c || d)) { cout << "NO" << endl; continue; }
		if (((abs(x2 - x1) < a || abs(x2 - x1) < b) && max(a,b) - min(a,b) > abs(x2 - x1)) ||
			((abs(y2 - y1) < c || abs(y2 - y1) < d) && max(c,d) - min(c,d) > abs(y2 - y1))) 
			cout << "NO" << endl;
		else cout << "YES" << endl;
	}
