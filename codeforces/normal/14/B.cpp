...
#define sll set<ll>
#define cinvec(v, n) f(i, 0, n) cin >> v[i];
...
int main() {
    ...
    int n, x;
    cin >> n >> x;
    int mxLeft = -1, mxRight = intinf;
    f(i, 0, n){
        int a, b; cin >> a >> b;
        mxLeft = max(mxLeft, min(a, b));
        mxRight = min(mxRight, max(a, b));
    }
    //cout << mxLeft << " " << mxRight;

    if (mxRight < mxLeft) cout << -1;
    else if (x >= mxLeft && x <= mxRight) cout << 0;
    else if (x <= mxLeft && x <= mxRight) cout << abs(mxLeft - x);
    else cout << min(abs(mxLeft - x), abs(mxRight - x));
