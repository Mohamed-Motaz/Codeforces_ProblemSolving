...
int solve(int idx, int remDough){
    if (idx == m) return 0;
    int &ans = dp[idx][remDough];
    if (ans != -1) return ans;

    ans = 0;

    int mxCurStuffing = brr[idx][0] / brr[idx][1];
    int mxWithoutStuffing = remDough / c0;

    f(i, 0, mxCurStuffing + 1){
            if (remDough - (i * brr[idx][2])>= 0) {
                ans = max(ans,
                          (int) i * brr[idx][3]  + solve(idx + 1, remDough - (i * brr[idx][2])));
            }
    }
    return ans;
}
...
int main() {
    ...
    f(i, 0, tmp + 1){
        if (c0 * i <= n){
            clr(dp, -1);
            mx = max(mx, d0 * (int)i + solve(0, n - c0 * i));
        }
    }
