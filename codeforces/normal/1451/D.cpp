...
string sortString(string s){
    ...
    for (auto e : s) frq[e]++;
    for (auto e: s) vv.push_back({frq[e], e});
    sort(all(vv));
    for (auto e: vv) ret += e.second;
    return ret;
}

int main() {
    MohamedMotaz;
    //cout << sortString("aaabba") << " " << sortString("ddddcc");
    cin >> t;
    while (t--){
...
//swap and make sure each char in a is smaller than b


//8 3
//aaaaabcd
//aabbbbcd  case doesnt work with below implementation
