...
int main() {
    ...
    while (t--){
        cin >> n;
        vii v1(n), v2(n);
        cinvec(v1,n); cinvec(v2,n);
        int mn1 = intinf, mn2 = intinf;
        for (auto e: v1) mn1 = min(e, mn1);
        for (auto e: v2) mn2 = min(e, mn2);
        ll counter = 0;
        f(i, 0, n){
            if (v1[i] > mn1 && v2[i] > mn2){
                int mn = min(v1[i] - mn1, v2[i] - mn2);
                counter += mn;
                counter += v1[i] - mn1 - mn;
                counter += v2[i] - mn2 - mn;
            }else{
                if (v1[i] > mn1) counter += abs(v1[i] - mn1);
                if (v2[i] > mn2) counter += abs(v2[i] - mn2);
