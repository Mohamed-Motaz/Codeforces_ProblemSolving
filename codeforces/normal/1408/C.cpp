...
int t;
ll n, k, m, a, b, r, c, d, x, y, q, z, f, l;
vld v;

ld car1(ld mid){
    ld speed = 1, cur = 0, time = 0;
    //need to loop until mid and not cross it
    for (int i = 0; i < n && mid - v[i] >= 1e-9; i++){
        time += abs(v[i] - cur) / speed;
        speed++;
        cur = v[i];
    }
    time += (mid - cur) / speed;
...
ld car2(ld mid){
    ld speed = 1, cur = l, time = 0;
    //need to loop until mid and not cross it
    for (int i = n - 1; i >= 0 && v[i] - mid >= 1e-9; i--){
        time += abs(cur - v[i]) / speed;
        speed++;
        cur = v[i];
    }
    time += (cur - mid) / speed;
...
int main() {
    ...
    while (t--){
        ...
        ld st = 0, end = 1e18, mid, ans;
        f(i, 0, 300){
            mid = (st + end) / 2;
            ld f = car1(mid), s = car2(mid);
            //debug2(f, s);
            if (f - s >= 1e-9){
                ans = s, end = mid;
            }else st = mid;
