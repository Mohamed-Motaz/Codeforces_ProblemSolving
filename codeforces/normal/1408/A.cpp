...
set<ll> GetDivisors(ll val){
    set<ll> st;
    f(i, 2, sqrt(val) + 1) if (val % i == 0) st.insert(i), st.insert(val / i);
    return st;
}
...
int main() {
    ...
    while (t--){
        ...
        f(i, 0, n) cin >> v2[i];
        vii ans;
        //try with first element
        if (v1[0] != v1[1] ||
            v1[0] != v2[1] ||
            v1[0] != v3[1]) ans.push_back(v1[0]);
        else if (v2[0] != v1[1] ||
                 v2[0] != v2[1] ||
                 v2[0] != v3[1]) ans.push_back(v2[0]);
        else if (v3[0] != v1[1] ||
                 v3[0] != v2[1] ||
                 v3[0] != v3[1]) ans.push_back(v3[0]);
        f(i, 1, n - 1){
            if (v1[i] != ans.back() &&
            (v1[i] != v1[i + 1] || v1[i] != v2[i + 1] || v1[i] != v2[i + 1])) ans.push_back(v1[i]);
            else if (v2[i] != ans.back() &&
                     (v2[i] != v1[i + 1] || v2[i] != v2[i + 1] || v2[i] != v2[i + 1])) ans.push_back(v2[i]);
            else if (v3[i] != ans.back() &&
                     (v3[i] != v1[i + 1] || v3[i] != v2[i + 1] || v3[i] != v2[i + 1])) ans.push_back(v3[i]);
        }
        int i = n - 1;
        if (v1[i] != ans.back()) ans.push_back(v1[i]);
        else if (v2[i] != ans.back()) ans.push_back(v2[i]);
        else if (v2[i] != ans.back()) ans.push_back(v3[i]);
        coutvec(ans);
    }
