...
int dp[505][505][505];
vii v;
int solve(int idx, int curX, int prev){
    if (idx > n) return 0;
    int &ans = dp[idx][curX][prev];
    if (ans != -1) return ans;
    ans = 1e8;

    if (curX < v[idx] && curX >= prev) ans = min(ans, 1 + solve(idx + 1, v[idx], curX));
    if (v[idx] >= prev) ans = min(ans, solve(idx + 1, curX, v[idx]));
    return ans;
}
...
int main() {
    ...
    while (t--){
        ...
        int ans = solve(1, x, 0); trace1(ans);
        if ( ans == 1e8) cout << -1;
        else cout << ans;
        cout << endl;
    }
