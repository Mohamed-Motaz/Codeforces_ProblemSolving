...
int main() {
    ...
    while (m--){
        int a, b, c;
        cin >> a >> b >> c;
        if (!arr[a] && !arr[b] && !arr[c]) {arr[a] = 1, arr[b] = 2, arr[c] = 3; continue;}
        if (arr[a]){
            //previous
            if (arr[a] == 1) arr[b] = 2, arr[c] = 3;
            if (arr[a] == 2) arr[b] = 1, arr[c] = 3;
            if (arr[a] == 3) arr[b] = 1, arr[c] = 2;
        }
        else if (arr[b]){
            //previous
            if (arr[b] == 1) arr[a] = 2, arr[c] = 3;
            if (arr[b] == 2) arr[a] = 1, arr[c] = 3;
            if (arr[b] == 3) arr[a] = 1, arr[c] = 2;
        }
        else if (arr[c]){
            //previous
            if (arr[c] == 1) arr[b] = 2, arr[a] = 3;
            if (arr[c] == 2) arr[b] = 1, arr[a] = 3;
            if (arr[c] == 3) arr[b] = 1, arr[a] = 2;
        }
    }
