...
ll t;
ll n, k, m, a, b, r, d, x, y, q, z, f, l, p, g;
vll R, G, B;
ll dp[N][N][N];
ll solve(int rr, int gg, int bb){
    if (rr >= r && gg >= g && bb >= b) return 0;
    if (rr >= r && gg >= g) return 0;
    if (rr >= r && bb >= b) return 0;
    if (gg >= g && bb >= b) return 0;


    ll &ans = dp[rr][gg][bb];
    if (ans != -1) return ans;
    ans = 0;
    if (rr < r && gg < g) ans = max(ans, R[rr] * G[gg] + solve(rr + 1, gg + 1, bb));
    if (rr < r && bb < b) ans = max(ans, R[rr] * B[bb] + solve(rr + 1, gg, bb + 1));
    if (gg < g && bb < b) ans = max(ans, G[gg] * B[bb] + solve(rr, gg + 1, bb + 1));
