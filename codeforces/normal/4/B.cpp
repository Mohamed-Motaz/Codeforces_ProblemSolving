...
int main() {
    ...
    int d, sumTime;
    cin >> d >> sumTime;
    int mnTotal = 0, mxTotal = 0;
    vpii v(d);
    f(i, 0, d) cin >> v[i].first >> v[i].second, mnTotal += v[i].first, mxTotal += v[i].second;

    if (mxTotal < sumTime || mnTotal > sumTime) return cout << "NO", 0;

    cout << "YES" << endl;
    int toFill = sumTime - mnTotal;
    for (auto e: v){
        if (toFill <= e.second - e.first){
            cout << e.first + toFill << " ";
            toFill = 0;
        }else cout << e.second << " ", toFill -= e.second - e.first;
    }
