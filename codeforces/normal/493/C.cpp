...
int main() {
    MohamedMotaz;
    cin >> n;
    vll v1(n + 1, 0); f(i, 0, n) cin >> v1[i];
    v1[n] = intinf;
    cin >> m;
    vll v2(m); cinvec(v2);
    sort(all(v1));
    sort(all(v2));
    pii sol = {-intinf, -intinf};
    f(i, 0, n + 1){
        ll start = 0, end = m - 1, mid, ans = -1;
        while (start <= end){
            mid = (start + end) / 2;
            if (v1[i] > v2[mid]) start = mid + 1, ans = mid;
            else end = mid - 1;
        }
        //debug1(mid);

        int first = i * 2 + (n - i) * 3;
        int second =  (ans + 1) * 2 + 3 * (m - ans - 1);
        //debug2(first, second);
        if (sol.first == -intinf || first - second > sol.first - sol.second ||
                (first - second == sol.first - sol.second && first > sol.first))
