...
bool hasChildren(trie * cur){
    for (auto e: cur->children){
        if (e) return true;
    }
    return false;
}
int countTrue(trie * root){
    if (!hasChildren(root)) return 1;
    int ans = 1;
...
int main() {
    ...
    f(i, 0, s.size()){
        ...
        f(j, i, s.size()){
            ctr += 1 - (t[s[j] - 'a'] - '0');
            if (ctr > k) break;  //avoid creating new tries that are leaves
            if (!cur->children[s[j] - 'a']){
                cur->children[s[j] - 'a'] = new trie();
            }
