...
int main() {
    ...
    if (both.size() + alice.size() < k || both.size() + bob.size() < k) return cout << -1, 0;

    vll sumA(alice.size() + 1, 0), sumB(bob.size() + 1, 0);
    f(i, 0, alice.size()) sumA[i + 1] = alice[i] + sumA[i];
    f(i, 0, bob.size()) sumB[i + 1] = bob[i] + sumB[i];
    
    ll ans = llinf, sum = 0;
    f(i, 0, both.size() + 1){
        if (i > 0) sum += both[i - 1];
        int need = k - i;
        if (need >= 0 && need <= alice.size() && need <= bob.size()) ans = min(ans, sum + sumA[need] + sumB[need]);
    }
    cout << ans;
