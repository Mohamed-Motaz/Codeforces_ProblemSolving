...
bool checkIfCan(ll sum, vll &vec){
    bool one = false, two = false, three = false;
    for (auto e: vec){
        if (e == sum * 1 / 3) one = true;

        if (e == sum * 2 / 3) two = true;
        if (e == sum) three = true;
    }
...
int main() {
    ...
    vll v(n);
    ll sum = 0;
    f(i, 0, n) cin >> a, sum += a, v[i] = a;
    if (sum % 3 != 0) return cout << 0, 0;

    vll cum(n, 0); cum[0] = v[0];
    f(i, 1, n) cum[i] += cum[i - 1] + v[i];
    if (!checkIfCan(sum, cum)) return cout << 0, 0;
    ll c = 0, t = 0;
