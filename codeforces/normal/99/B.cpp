...
int main() {
    ...
    mii mp;
    for (auto e: v) mp[e]++;
    if (mp.size() == 1) return cout << "Exemplary pages.", 0;
    if (mp.size() > 3) return cout << "Unrecoverable configuration.", 0;

    int mn = intinf, fIdx = -1, sIdx = -1, mx = 0;
    f(i, 0, n){
        if (v[i] < mn){
            mn = v[i];
            fIdx = i + 1;
    ...
    int total = mn + mx;
    if (total / 2 != ceil(total / 2.0)) return cout << "Unrecoverable configuration.", 0;
    mp[total / 2]++;
    mp[mn]--;
    mp[mx]--;
    int sz = 0;
    ...
    if (sz == 1) return cout << (mx - mn ) / 2 << " ml. from cup #" << fIdx << " to cup #" << sIdx << ".", 0;
