...
int main() {
    ...
    while (true){
        //cout << ctr << endl;
        bool found = false;
        vpii toRemove; //node, leaf
        f(i, 1, n + 1){
            if (adjList[i].size() == 1){
                //cout << "found" << endl;
                found = true;
                toRemove.push_back({adjList[i][0], i});
                //adjList[adjList[i][0]].erase(find(all( adjList[adjList[i][0]]), i));
                adjList[i].pop_back();
            }
        }
        for (auto e: toRemove){
            if (adjList[e.first].size()) adjList[e.first].erase(find(all(adjList[e.first]), e.second));
        }
