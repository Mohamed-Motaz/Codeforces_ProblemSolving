...
int main() {
    ...
    vpii ans;
    for (auto e: mp){
        if (e.second.size() == 1) ans.push_back({e.first, 0});
        else{
            int diff = e.second[1] - e.second[0];
            bool can = true;
            f(i, 1, e.second.size() - 1){
                if (e.second[i + 1] - e.second[i] != diff) can = false;
            }
            if (can) ans.push_back({e.first, diff});
        }
    }
    cout << ans.size() << endl;
