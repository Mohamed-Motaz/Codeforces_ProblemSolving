...
bool check(vector<string> vec, string name){
    for (auto e: vec){
...
int main() {
    ...
    f(i, 0, n){
        ...
        while (cin >> tmp){
            ctr++;
            words.push_back(tmp);
            if (ctr > 3 && (tmp == "post" || tmp == "wall")) break;
        }
        //for (auto e: words) cout << e << " "; cout << endl;
        //remove the 's from the names
        words[words.size() - 2].pop_back();
        words[words.size() - 2].pop_back();
        if (words[words.size() - 2] < words[0]) swap(words[0], words[words.size() - 2]);

        if (!check(words, name)) {
            string tmpName1 = name, tmpName2 = name;
            if (tmpName1 < words[0]) swap(words[0], tmpName1);
            if (!mp[{words[0], tmpName1}]) mp[{words[0], tmpName1}] = 0;

            if (tmpName2 < words[words.size() - 2]) swap(words[words.size() - 2], tmpName2);
            if (!mp[{words[words.size() - 2], tmpName2}]) mp[{words[words.size() - 2], tmpName2}] = 0;

            continue;
        }
        if (words[1] == "posted") mp[{words[0], words[words.size() - 2]}] += 15;
        if (words[1] == "commented") mp[{words[0], words[words.size() - 2]}] += 10;
        if (words[1] == "likes") mp[{words[0], words[words.size() - 2]}] += 5;
    }
    vector<pair<int, string>> ans;
    for (auto e: mp){
        //cout << e.first.first << " " << e.first.second << " " << e.second << endl;
        if (e.first.first != name){
            ans.push_back({e.second, e.first.first});
        }else if (e.first.second != name){
            ans.push_back({e.second, e.first.second});
        }
            //if (e.first.first != name) cout << e.first.first << endl;
            //if (e.first.second != name) cout << e.first.second << endl;

        //cout << e.second << endl;
    }

    sort(all(ans), comp);
    for (auto e: ans) cout << e.second << endl;
