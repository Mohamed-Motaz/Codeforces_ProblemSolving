...
int main() {
    ...
    cin >> n >> k;
    vll v(n);
    f(i, 0, n) cin >> v[i], idx[v[i]].push_back(i);
    ll ans = 0;

    f(i, 0, n){
        if (v[i] % k == 0){
            ll tmp1 = v[i] / k, tmp2 = v[i] * k;
            auto it = lower_bound(all(idx[tmp1]), i);
            ll first = it - idx[tmp1].begin();
            auto it2 = upper_bound(all(idx[tmp2]), i);
            if (it2 == idx[tmp2].end()) continue;
            ll second = idx[tmp2].size() - (it2 - idx[tmp2].begin());
            ans += second * first;
        }
    }
    cout << ans;
