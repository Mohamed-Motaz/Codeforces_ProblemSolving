...
void changeToLower(string &f){
    f(i, 0, f.size()) f[i] = (char)tolower(f[i]);

}

void removeSigns(string &f){
    f(i, 0, f.size()){
        if (f[i] == '_' || f[i] == ';' || f[i] == '-') f.erase(f.begin() + i), i--;
    }
}
...
int main() {
    ...
    removeSigns(v[0]), removeSigns(v[1]), removeSigns(v[2]);
    changeToLower(v[0]), changeToLower(v[1]), changeToLower(v[2]);
    //cout << f << " " << s << " " << t << endl;
    ...
    while (n--){
        string ans; cin >> ans;
        removeSigns(ans), changeToLower(ans);
        bool can = false;
        for (auto e: permutations){
            if (e == ans) can = true;
        }

        if (can) cout << "ACC"; else cout << "WA"; cout << endl;
    }
