...
int main() {
    ...
    f(i, 0, m) f(j, 0, n) cin >> arr[i][j];
    f(i, 0, m) f(j, 0, n) ans[i][j] = 1;
    ...
    f(i, 0, m){
        f(j, 0, n){
            bool found = false;
            if (arr[i][j] == 1){
                //need to check if at least 1 is present in any col or row
                f(k, 0, m){
                    if (ans[k][j] == 1) found = true;
                }
                f(k, 0, n){
                    if (ans[i][k] == 1) found = true;
                }
                if (!found){
                    canDo = false;
                    break;
