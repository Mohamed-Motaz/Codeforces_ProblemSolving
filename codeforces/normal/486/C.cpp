...
int main() {
    ...
    cin >> n >> p >> s;
    if (p > n / 2) p = n - p + 1, reverse(all(s));
    ll c = 0, l = -1, r = -1;
    p--; //work with indexes
    f(i, 0, n / 2){
        if (s[i] != s[n - i - 1]){
    ...
    if (l == -1 && r == -1) return cout << 0, 0;


    if (r == -1) {
        //no r, only l
        c += p - l ;
        c += min(abs(s[l] - s[n - l - 1]), (26 - abs(s[l] - s[n - l - 1])));
    ...
    else c += min(max(r - p + r - l , 0ll), max(p - l + r - l , 0ll));
    //cout << l << " " << r << " " << c << endl;
