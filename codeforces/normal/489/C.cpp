...
//        for (i = 1; i <= N; i++) {
//            for (j = 1; j <= N; j++) {
//                if (distancee[i][k] + distancee[k][j] < distancee[i][j]) {
//                    distancee[i][j] = distancee[i][k] + distancee[k][j];
//                    paths[i][j] = k;
//                }
//            }
//        }
...
int main() {
    int m, s, tmp = s;
    cin >> m >> s;
    if ((m > 1 && s == 0) || m * 9 < s) return cout << "-1 -1", 0;
    if (m == 1) return cout << s << " " << s, 0;
    vii sol;
    while (m--){
        if (s >= 9) sol.push_back(9), s -= 9;
        else sol.push_back(s), s = 0;
    }
    sort(rall(sol));
    vii mn;
    //min
    fr(i, sol.size() - 1, 0) mn.push_back(sol[i]);
    int idx;
    f(i, 0, mn.size()) if (mn[i] != 0) {idx = i; break;}
    if (mn[0] == 0) mn[0] = 1, mn[idx]--;
    for (auto e: mn) cout << e; cout << " ";
    //max
    for (auto e: sol) cout << e; cout << " ";
