...
int main() {
    ...
    f(i, 1, 101){
        ...
        f(j, 0, v.size()){
            ...
            int first = *e.begin();
            int last = *e.rbegin();
            if (first >= numberPerFloor * (j - 1) + 1 && last <= numberPerFloor * j) continue;
            else{
                can = false;
                break;
            }
        }
        if (can) sol.push_back(i);
    }
    set<int> vals;
    for (auto e: sol) vals.insert(ceil(n / (double)e));
    if (vals.size() > 1) return cout << -1, 0;
    cout << *vals.begin();
