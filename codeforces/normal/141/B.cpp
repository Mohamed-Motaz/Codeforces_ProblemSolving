...
int main(){
    ...
    if (val < 2 && fabs((ll)val - val) != 0 && x > -(ld)(a / 2.0) && x < (ld)(a / 2.0)){
        return cout << ceil(val), 0; //in the first two bars;
    }
    val--;
    //remove first one and check the seq
    ll lvl = (ll)val % 2;
    ll brick = ceil(val / 2.0);
    ll fin = brick * 3 - 1;
    if (lvl)
        if (x < 0){ fin++;}
        else {fin+= 2;}
