...
int solve(int curTime, int curSpeed){
    if (curTime == t){
        //reached the end
        return (curSpeed == v2) ? v2 : -intinf;
    }
    if (dp[curTime][curSpeed] != -1) return dp[curTime][curSpeed];
    int ans = -intinf;
    f(i, curSpeed - d, curSpeed + d + 1){
        if (i >= 0)
        ans = max(ans, curSpeed + solve(curTime + 1, i));
    }
    return dp[curTime][curSpeed] = ans;
}
