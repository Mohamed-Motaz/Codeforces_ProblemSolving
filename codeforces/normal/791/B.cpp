...
bool check(set<int> &s1, set<int> &s2){
    ...
    for (auto e: s1){
        //cout << e << " ";
        if (s2.find(e) == s2.end()) ctr++;
    }
    //cout << endl;
    for (auto e: s2){
        //cout << e << " ";
        if (s1.find(e) == s1.end()) ctr++;
    }
    //cout << endl;
    //cout << ctr << endl;
    return ctr == 2;
...
void dfs(int node){
    ...
    for (auto e: adjList[node]){
        if (!check(adjList[node], adjList[e])){
            cout << "NO";
            exit(0);
        }
        dfs(e);
...
int main(){
    ...
    for (int i = 1; i <= n; i++){
        if (!vis[i]){ //make sure every component is connected to everyone
            dfs(i);
