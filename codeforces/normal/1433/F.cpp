...
int solve(int row, int col, int remaining, int sum){
    if (row == n) return sum == 0 ? 0 : -1e8; //last row
    if (col == m) return solve(row + 1, 0, m / 2, sum);
    int &ans = dp[row][col][remaining][sum];
    if (ans != -1) return ans;
    //leave
    ans = solve(row, col + 1, remaining, sum);
    //take
    if (remaining - 1 >= 0) ans = max(ans, solve(row, col + 1, remaining - 1, (sum + brr[row][col]) % k) + brr[row][col]);
    return ans;
}
