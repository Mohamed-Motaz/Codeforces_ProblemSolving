...
int main() {
    ...
    while (t--){
        ...
        vpll v(n);
        mll mp;
        f(i, 0, n) cin >> v[i].first, v[i].second = i + 1 , mp[v[i].first]++;
        if (mp.size() == 1){
            cout << "NO" << endl;
            continue;
        }
        cout << "YES" << endl;
        sort(all(v));
        int cur = v[0].first, curIdx = v[0].second, idx = 1;
        //connect all unlike him to him
        while (v[idx].first == cur) idx++;
        while (idx < n){
            cout << curIdx << " " << v[idx].second << endl;
            idx++;
        }
        int num = 1;
        while (v[num].first == v[0].first){
            cout << v[num].second << " " << v[n - 1].second << endl;
            num++;
        }
...
//        }
//
//        //check if any left, and connect them to nearby ones
//        int num = 1;
//        while (v[num] == v[0]){
//            cout << (num + 1)<< " " << n << endl;
//            num++;
