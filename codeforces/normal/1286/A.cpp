...
vii v;
                    ...
                    //1 is odd, 0 is even
int solve(int idx, int prev, int odds, int evens){
    if (idx == n) return 0;
    if (dp[idx][prev][odds][evens] != -1) return dp[idx][prev][odds][evens];

    int ans = intinf;

    if (v[idx]){
        //element present
        //1: first element so no parity
        //2: not first so there is parity
        ans = min(ans, solve(idx + 1, v[idx] % 2, odds, evens) + (prev != 3 && v[idx] % 2 != prev));
    }else{
        if (odds){
            ans = min(ans, solve(idx + 1, 1, odds - 1, evens) + (prev == 0));
        }
        if (evens){
            ans = min(ans, solve(idx + 1, 0, odds, evens - 1) + (prev == 1));
        }
    }

    return dp[idx][prev][odds][evens] = ans;
...
int main() {
    ...
    f(i, 0, n){
        cin >> v[i];
        if (v[i]) v[i] % 2 ? odds++: evens++;
    }
    //debug2(odds, evens);
    //cout << n / 2 - evens << " " << ceil(n / 2.0) - odds;

    memset(dp, -1, sizeof(dp));
    cout << solve(0, 3, ceil(n / 2.0) - odds, n / 2 - evens);
