...
int solve(int idx, int cur){
    ...
    if (cur == 1){
        ans = solve(idx + 2, cur ^ 1);
    }
    if (!cur){
        //take
        if (v[idx] == 1) {
            ans = 1 + solve(idx + 1, cur ^ 1);
            // 1 0
            if (idx + 1 < n && v[idx + 1] == 0) ans = 1 + solve(idx + 2, cur ^ 1);
                // 1 1
            else if (idx + 1 < n && v[idx] == 1){
                ans = min({ans,
                           2 + solve(idx + 2, cur ^ 1), //take both ones
                           1 + solve(idx + 1, cur ^ 1)} //take only first one
                );
            }

        }
        else if (v[idx] == 0){
            ans = solve(idx + 1, cur ^ 1);
            //0 1
            if (idx + 1 < n && v[idx + 1] == 1){
                ans = min(ans, 1 + solve(idx + 2, cur ^ 1));
            }
                //0 0
            else if (idx + 1 < n && v[idx + 1] == 0) ans = min({
                                                                       ans,
                                                                       solve(idx + 2, cur ^ 1),
                                                                       solve(idx + 1, cur ^ 1)});
        }
