...
int main() {
    cin >> n;
    vii v(n), numberVeryPleased(200007, 0);
    cinvec(v);
    mii indexes;
    for (auto e: v) indexes[e]++;
    cin >> m;
    vpii movies(m);
    f(i, 0, m) cin >> movies[i].first;
    f(i, 0, m) cin >> movies[i].second;

    int mxVeryPleased = -intinf + 5, mxIdx = -1;

    f(i, 0, m){
        if (indexes[movies[i].first] > mxVeryPleased){
            mxVeryPleased = indexes[movies[i].first], mxIdx = i;
        }
    }
    bool foundAgain = false;
    f(i, 0, m){
        if (indexes[movies[i].first] == mxVeryPleased && i != mxIdx) foundAgain = true;
    }
    //debug2(mxVeryPleased, mxIdx);
    if (!foundAgain) return cout << ++mxIdx, 0;
    //cout << "in" << endl;
    int mxAlmostPleased = -intinf + 5, mxIDX = -1;
    //foundagain
    f(i, 0, m){
        if (indexes[movies[i].first] == mxVeryPleased){
            if (indexes[movies[i].second] > mxAlmostPleased){
                mxAlmostPleased = indexes[movies[i].second];
                mxIDX = i;
            }
        }
    }
