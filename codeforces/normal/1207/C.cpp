...
int main()
{
    ...
    while (t--){
        ...
        dp[0][0] = b; dp[0][1] = b + b + a; dp[1][1] = b + (ll)2 * a + (ll)2 * b;
        f(i,1,n){
            if (s[i] == '0'){

                //to first lvl   at lvl2             at lvl 1 previously
                dp[0][i + 1] = min(dp[1][i] + (ll)2 * a + b,dp[0][i] + a + b);
                // to second lvl at lvl 2            from lvl 1
                dp[1][i + 1] = min(dp[1][i] + a + (ll)2 * b, dp[0][i] + (ll)2 * a + (ll)2 * b);

            }else{
                dp[0][i + 1] = INF; //very important line that caused my bug
                dp[1][i + 1] = dp[1][i] + a + (ll)2 * b;
            }
        }
