...
int main() {
    ...
    cin >> s;
    bool neg = false;
    if (s[0] == '-') neg  = true, s.erase(s.begin()); //remove the neg
    bool dotFound = false;
    for (auto e: s) if (e == '.') dotFound = true;

    if (!dotFound) s += ".00";
    fr(i, s.size() - 1, 0){
        if (s[i] == '.'){
            if (s.size() - i == 3) break; //correct
            if (s.size() - i < 3){
                //add zeros;
                int toAdd = 3 - (s.size() - i);
                while (toAdd--) s += '0';
            }else{
                //remove zeros
                int toRemove = (s.size() - i) - 3;
                while (toRemove--) s.pop_back();
            }
            //cout << i << " " << s.size() << endl;
            break;
        }
