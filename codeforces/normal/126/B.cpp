...
int main() {
    ...
    set<string> values;
    for (auto e: mp){
        if (e.second > 1 && e.first.size() != s.size()) can = true, values.insert(e.first);
    }
    if (!can) return cout << "Just a legend", 0;
    string mx = "";
    for (auto e: values){
        f(i, 1, s.size() - e.size()){
            if (s.substr(i, e.size()) == e){
                if (e.size() > mx.size()) mx = e;
            }
        }
    }
    if (mx.size() == 0) return cout << "Just a legend", 0;
    cout << mx;
