...
int main() {
    ...
    while (t--) {
        ...
        cin >> s;
        vll sub;
        if (s[0] == 'W') sub.push_back(1);
        else sub.push_back(-1);
        char cur = s[0];
        f(i, 1, n) {
            if (s[i] == cur) {
                if (cur == 'W') sub[sub.size() - 1] += 2;
                else sub[sub.size() - 1] -= 1;
            } else {
                cur = s[i];
                if (cur == 'W') sub.push_back(1);
                else sub.push_back(-1);
            }
        }
        if (n == 1){
        ...
        ll sum = 0;
        for (auto e: sub) (e > 0) ? sum += e : sum;

        //get all negatives except first and last
        priority_queue<ll> pq;
        f(i, 1, sub.size() - 1){
            if (sub[i] < 0) pq.push(sub[i]);
        }
        ...
        //check first and last ones now
        if (sub[0] < 0 && k){
            ll mn = min(k, -sub[0]);
            sum += mn * 2;
            k -= mn;
        }
        if (sub[sub.size() - 1] < 0 && k){
            ll mn = min(k, -sub[sub.size() - 1]);
            sum += mn * 2;
            k -= mn;
        }
