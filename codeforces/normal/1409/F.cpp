...
int solve(int idx, int prev, int rem){
    if (idx == n) return 0;

    int &ans = dp[idx][prev][rem];
    if (ans != -1) return ans;
    ans = 0;

    //try to take1
    if (s[idx] == t[0])
        ans = max(ans, solve(idx + 1, prev + 1, rem));
    else if (rem > 0) ans =  max(ans, solve(idx + 1, prev + 1, rem - 1));

    //try to take2
    if (s[idx] == t[1])
        ans = max(ans, prev + solve(idx + 1, prev, rem));
    else if (rem > 0) ans = max(ans, prev + solve(idx + 1, prev, rem - 1));

    //leave
    ans = max(ans, solve(idx + 1, prev, rem));
...
int main() {
    ...
    if (t[0] == t[1]){
        int ctr = 0;
        for (auto e: s) ctr += e == t[0];
        ctr += k;
        //cout << ctr << endl;
        return cout << min(n * (n - 1) / 2, ctr * (ctr - 1) / 2), 0;
    }
    cout << solve(0, 0, k);
