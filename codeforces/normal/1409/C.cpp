...
int main() {
    ...
    while (t--){
        ...
        if (diff % (n - 1) == 0){
            ...
            ll first = x;
            n -= 1;
            f(i, 1, n) cout << first + diff << " ", first += diff;
            cout << y << endl;
        }else{
            sll st = GetDivisors(diff);
            ll valueFound = -1;
            set<ll>::reverse_iterator e;
            for (e = st.rbegin(); e != st.rend(); e++) {
                    if (diff / *e <= (n - 2)) {
                        valueFound = *e;
                    }

            }
            if (valueFound == -1) valueFound = diff;
            //cout << "in" <<  valueFound << endl;
            //cout << x << " " << y << " ";
            for (ll i = x; i <= y; i += valueFound){
                cout << i << " ", n--;
            }
            //n -= 2;
            while(n > 0){
