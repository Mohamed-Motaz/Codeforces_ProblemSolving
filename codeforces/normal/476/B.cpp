...
int finalPos = 0;
vii v;
void solve(int idx, int currPos){
    if (idx == s.size()){
        v.push_back(currPos);
        return;
    }
    if (t[idx] == '+') solve(idx + 1, currPos + 1);
    else if (t[idx] == '-') solve(idx + 1, currPos - 1);
    else{
       solve(idx + 1, currPos - 1);
       solve(idx + 1, currPos + 1);
    }
}
...
int main() {
    cin >> s >> t;
    for (auto e: s) (e == '+') ? finalPos++ : finalPos--;
    solve(0, 0);
    ll tmp = 0;
    for (auto e: v) tmp += e == finalPos;

    cout << fixed << setprecision(10) << (ld)tmp / (ld)v.size();
}