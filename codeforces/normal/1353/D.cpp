...
int main() {
    ...
    while (t--){
        ...
        while (pq.size()){
            ...
            int tmp = (p.second.first + p.second.second) / 2;
            v[tmp] = ctr++;

            if (p.second.first <= tmp - 1 ) pq.push({{abs( -p.second.first - -(tmp - 1))}, { -p.second.first , -(tmp - 1)}});
            if (tmp + 1 <= p.second.second)pq.push({abs(-(tmp + 1) - -p.second.second), {-(tmp + 1) , -p.second.second}});
