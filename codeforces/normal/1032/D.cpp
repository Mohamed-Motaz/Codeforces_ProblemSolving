...
int main() {
    ...
    point p1(x1, y1), p2(x2, y2), t1, t2, t3, t4;
    ld dist = abs(x1 - x2) + abs(y1 - y2);
    //cout << dist << endl;
    //2 ways, either take direct route from avenue
    //move to x, then take route, then to y

    if (!a || !b) return cout << dist, 0;

    if (a){
        t1 = point(((-c - (b * y1)) / a), y1);
        t2 = point(((-c - (b * y2)) / a), y2);
    }
    if (b){
        t3 = point(x1, (-c - (a * x1)) / b);
        t4 = point(x2, (-c - (a * x2)) / b);
    }
//    cout << t1 << " " << t2 << " " << t3 << " " << t4 << endl;
//
//    cout << abs(p1 - t1) + abs(t1 - t2) + abs(t2 - p2) << endl;
//    cout << abs(p1 - t1) + abs(t1 - t4) + abs(t4 - p2) << endl;
//    cout << abs(p1 - t3) + abs(t3 - t2) + abs(t2 - p2) << endl;
//    cout << abs(p1 - t3) + abs(t3 - t4) + abs(t4 - p2) << endl;


    dist = min({dist ,
                abs(p1 - t1) + abs(t1 - t2) + abs(t2 - p2),
                abs(p1 - t1) + abs(t1 - t4) + abs(t4 - p2),
                abs(p1 - t3) + abs(t3 - t2) + abs(t2 - p2),
                abs(p1 - t3) + abs(t3 - t4) + abs(t4 - p2)
                });
    cout << dist;
    return 0;
}
