...
int main() {
    ...
    cin >> n;
    vpii v1(n);
    f(i, 0, n) cin >> v1[i].second >> v1[i].first, tmp = max(tmp, v1[i].second);
    cin >> m;
    vpii v2(m);
    f(i, 0, m) cin >> v2[i].first >> v2[i].second;

    sort(all(v1));
    sort(all(v2));

//    for (auto e: v1) cout << e.first << " " << e.second << endl;
//    cout << endl endl;
//    for (auto e: v2) cout << e.first << " " << e.second << endl;


    int mx = 0;
    if (v2[m - 1].first > v1[0].first) mx = abs(v1[0].first - v2[m - 1].first);


    if (tmp > v2[0].second) mx = max(abs(tmp - v2[0].second), mx);
