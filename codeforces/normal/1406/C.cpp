...
vii adjList[N];
vii dist(N), centroids;
int mn = intinf;
void init(){
    f(i, 1, n + 1) adjList[i].clear(), dist[i] = 0;
    mn = intinf;
    centroids.clear();
}
...
int dfs(int node, int parent){
    ...
    }else if (mx == mn) centroids.push_back(node);
    return dist[node];
}
...
int main() {
    ...
    while (t--){
        ...
        dfs(1, 0);
        //f(i, 1, n + 1) cout << dist[i] << " "; cout << endl;

        if (centroids.size() == 1){
            cout << adjList[1][0] << ' ' << 1 << endl;
            cout << adjList[1][0] << ' ' << 1 << endl;
        }else{
            //cout << centroids.size() << endl;
            for (auto e: adjList[centroids[0]]){
                if (e != centroids[1]){
                    cout << centroids[0] << " " << e << endl;
                    cout << e << " " << centroids[1] << endl;
                    break;
                }
            }
        }
