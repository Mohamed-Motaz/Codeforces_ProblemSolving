...
int main() {
    ...
    int p, q, l ,r;
    cin >> p >> q >> l >> r;
    vpii littlex(p), littlez(q);
    vii v(1007, 0);
    f(i, 0, p) {
        cin >> littlex[i].first >> littlex[i].second;
        f(j, littlex[i].first, littlex[i].second + 1) v[j] = 1;
    }
    f(i, 0, q) cin >> littlez[i].first >> littlez[i].second;

    int ctr = 0;


    for (int j = l; j <= r; j++){ //add time until it reaches
        //each iteration is a single shift
        f(i, 0, littlez.size()){
            bool found = false;
            //now check if with this shift their is still a way to meet
            for (int k = littlez[i].first + j; k <= littlez[i].second + j && k <= 1001; k++){
                if (v[k]){
                    found = true;
                    ctr++;
                    break;
