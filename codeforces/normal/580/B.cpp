...
int main(){
    ...
    cumSum[0] = v[0].second;
    for (int i = 1; i < n; i++)
        cumSum[i] += cumSum[i - 1] + (ll)v[i].second;
    ll mx = 0;
//    for (int i = 0; i < n; i++){
//        cout << v[i].first << " " << v[i].second << " " << cumSum[i] << endl;
//    }
    for (int i = 0; i < n; i++){
        auto it = upper_bound(v.begin(), v.end(), make_pair(v[i].first + d, 0));
        if (it == v.end()) it--;
        int idx =  it - v.begin();
        if (v[idx].first >= v[i].first + d)
            idx--;
        if (i == 0)
            mx = max(mx, cumSum[idx]);
        else
            mx = max(mx, cumSum[idx] - cumSum[i - 1]);
