...
int main() {
    ...
    vpll v(n + 1);
    vll dist(n + 1, 0);
    f(i, 1, n + 1) cin >> v[i].first >> v[i].second, dist[i] += dist[i - 1] + v[i].first - v[i - 1].second;

    ll ans = 0;
    //printvec(dist);
    f(i, 1, n + 1){
        int idx = lower_bound(dist.begin() + 1, dist.end(), h + dist[i]) - dist.begin();
        idx--;
        //debug4(v[idx].second, v[i].first, dist[idx], dist[i]) << endl;
        ans = max(ans, v[idx].second - v[i].first + h - (dist[idx] - dist[i]));
