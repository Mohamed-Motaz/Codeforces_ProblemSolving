...
int main()
{
    ...
    {
        int k=0;
        if(q2[a[i]]>0)// If the current number can be directly used, it can be used directly. For example, to give an example, 1 10, this time 10 1 is definitely available.
        {
            int t1=q2[a[i]];
            int t2=a[i]/t1;
            ans[i].x=t1;
        ...
        else
        {
            ...
            {
                q[a[i]]++;
                if(q[a[i]]*q[a[i]]>a[i])// If it exceeds the root number of this number, there is definitely no suitable /.
                {
                    flag=0;
                    break;
                }
                if(a[i]%q[a[i]]==0)
                {
                    ans[i].x=q[a[i]];
                    ans[i].y=a[i]/q[a[i]];
                    if(ans[i].x!=ans[i].y)//. . Do not record duplicates
                            q2[a[i]]=a[i]/q[a[i]];
                    k=1;
                    break;
                }
