...
int t;
ll n, k, m, a, b, r, c, d, x, y, q, z, f;




#define read freopen("hello.in", "r", stdin);
int main() {
    MohamedMotaz;
    cin >> n;
    vii v(n + 1);
    map<int, vii> mp; //num and indices
    f(i, 1, n + 1){
        cin >> v[i];
        mp[v[i]].push_back(i);
    }
    map<pii, int> pairsPresent;
    cin >> m;
    f(i, 0, m){
        cin >> a;
        vii idices = mp[a];
        if (idices.size() == 0) continue;
        int ptr1 = mp[a][0]; // first instance;
        int ptr2 = mp[a][mp[a].size() - 1]; //last instance

        if (ptr1 == ptr2) continue; //only one element present
        for (auto e: pairsPresent){
            if (ptr1 < e.first.first && ptr2 > e.first.second){//segment found consumes current one, we need to remove the current one
                pairsPresent[e.first] = 0;
                break;
            }
            if (!e.second) continue;
            if (ptr1 > e.first.first && ptr1 < e.first.second) {  //shift ptr1 to the right
                int idx = 1;
                while (ptr1 < e.first.second && idx < idices.size()){
                    ptr1 = idices[idx++];
                }
            }
            if (ptr2 > e.first.first && ptr2 < e.first.second) { //shift ptr2 to the left
                int idx = idices.size() - 2;
                while (ptr2 > e.first.first && idx >= 0){
                    ptr2 = idices[idx--];
                }
            }
            if (ptr2 <= ptr1 || ptr2 < 1 || ptr1 > n) break;
            //check if ok after change



        }
        if (ptr2 <= ptr1 || ptr2 < 1 || ptr1 > n) continue;
        pairsPresent[{ptr1, ptr2}] = a;
    }
    for (auto e: pairsPresent){
        if (e.second == 0) continue;
        int l = e.first.first, r = e.first.second;
        f(i, l, r + 1) v[i] = e.second;
    }
