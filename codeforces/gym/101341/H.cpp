...
ll t;
ll n, k, m, a, b, r, d, x, y, q, z, f, l, p, u, v;
...
int main() {
    ...
    //debug2(mxRow, mxCol);
    pii ans;
    arr[mxRow][mxCol] = 0; //set max to 0;
    //loop on max row and col, and see the max met
    int tmpMxRow = 0, tmpMxCol = 0;
    f(i, 1, m + 1){
        tmpMxRow = max(arr[mxRow][i], tmpMxRow);
    }
    f(i, 1, n + 1){
        tmpMxCol = max(arr[i][mxCol], tmpMxCol);
    }
    //cout << tmpMxCol << " " << tmpMxRow << endl;
    if (tmpMxRow > tmpMxCol){
        //erase all rows
        ans.first = mxRow;
        f(i, 1, m + 1){
            arr[mxRow][i] = 0;
        }
        //find max num in which col
        mx = 0;
        f(i, 1, n + 1) {
            f(j, 1, m + 1) {
                if (arr[i][j] > mx){
