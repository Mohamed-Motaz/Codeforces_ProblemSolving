...
int main() {
    ...
    for(int i=0; i<s.size(); i++)
    {
        string t = s.substr(i, happy.size());
        if (t == happy)
        {
            ctr++, idx.push_back(i);
    ...
    if (ctr == 0)
    {
        ...
        cout << "YES" << endl;
        string tmp = s;
        swap(tmp[0], tmp[s.size() - 1]);
        bool oops = false;
        for(int i=0; i<tmp.size(); i++)
        {
            string t = tmp.substr(i, happy.size());
            if (t == happy)
                oops = true;
        }
        if (oops) cout << 1 << " " << 8;
    ...
    if      (ctr==2)cout<<"YES"<<endl<<idx[0] + 1<<" "<<idx[1] + 2;
    else if (ctr > 1) return cout << "NO", 0;
    else
    {
        cout << "YES" << endl;
        cout << idx[0] + 1 << " " << idx[0] + 2 << endl;
    }
