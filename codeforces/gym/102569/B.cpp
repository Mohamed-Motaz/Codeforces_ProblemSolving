...
int main() {
    ...
    f(i, 0, pos.size()){
        if (pos[i] <= cnt){
            //move to the right in the pos, then to the left in the neg
            res++;
            ll tmp = cnt - pos[i];
            ll ans = tmp - pos[i];
            if (ans >= 0){  //can turn back to the left
                int idx = upper_bound(all(neg), ans) - neg.begin();
                mx = max(mx, idx + res + zero);
            }
        }else break;
    }
    mx = max(mx, res + zero);
    ...
    f(i, 0, neg.size()){
        ...
        if (neg[i] <= cnt){
            //move to the left in the neg, then to the right in the pos
            res++;
            ll tmp = cnt - neg[i];
            ll ans = tmp - neg[i];
            if (ans >= 0){  //can turn back to the left
                int idx = upper_bound(all(pos), ans) - pos.begin();
                mx = max(mx, idx + res + zero);
            }
        }else break;
    }
    cout << max(mx, res + zero) << endl;
