...
int main() {
    ...
    int ctrCap = 0, ctrSmall = 0;
    int ans[11000];
    f(i, 0, 2 * n){
        pair<char, int> toPush;  //this is the large B
        char cur = s[i];
        toPush.first = cur;
        if (cur >= 'A' && cur <= 'Z'){
            toPush.second = ctrCap++;
        }
        if (cur >= 'a' && cur <= 'z'){
            toPush.second = ctrSmall++;
        }

        if (st.size()){
            pair<char, int> ttt = st.top();
            if (ttt.first == toPush.first + 32){
                ans[toPush.second] = ttt.second;
                st.pop();
            }else if (32 + ttt.first == toPush.first){
                ans[ttt.second] = toPush.second;
                st.pop();
            }else st.push(toPush);
        }else st.push(toPush);
