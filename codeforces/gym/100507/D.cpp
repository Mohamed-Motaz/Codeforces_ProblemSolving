...
//    void insertBefore(struct Node* next_node, int node_data)
//    {
//        struct Node* newNode =new Node;
//        newNode->next = next_node;
//        newNode->prev = next_node->prev;
//        if (next_node->prev != NULL){
//            if (next_node->prev->next != NULL)
...
ld mxToPayPerMonth, advantagesAloneOneBedroom, advantagesAloneTwoeBedroom;


struct room{
    ld number, cost, advantages;
};
struct friendd{
    ld pay, advantages;
};
vector<room> appartments;
vector<friendd> friends;


int main() {
    MohamedMotaz;
    cin >> mxToPayPerMonth >> advantagesAloneOneBedroom >> advantagesAloneTwoeBedroom;
    cin >> n;
    friends.resize(n);
    f(i, 0, n) cin >> friends[i].pay >> friends[i].advantages;
    cin >> m;
    appartments.resize(m);
    f(i, 0, m) cin >> appartments[i].number >> appartments[i].cost >> appartments[i].advantages;

    //cases
    //cant live in anything
    bool can = false;

    ll mxAdv = -intinf;
    ll alone = 0;
    ll friendNum = 0, friendRoom = 0;

    f(i, 0, m){  //check for alone in two or one bedroom
        if (mxToPayPerMonth >= appartments[i].cost){
            can = true;
            ll toAdd;
            if (appartments[i].number == 1) toAdd = advantagesAloneOneBedroom;
            else toAdd = advantagesAloneTwoeBedroom;
            if (toAdd + appartments[i].advantages > mxAdv){

                mxAdv = toAdd + appartments[i].advantages;
                alone = i + 1;
            }
        }
    }
    ...
    f(j, 0, m){
        f(i, 0, n){  //check for with people
            if (appartments[j].number == 2 &&
                mxToPayPerMonth * 2 >= appartments[j].cost &&
                friends[i].pay * 2 >= appartments[j].cost){
                can = true;
                if (appartments[j].advantages + friends[i].advantages > mxAdv){
                    alone = 0;
                    mxAdv = appartments[j].advantages + friends[i].advantages;
                    friendNum = i + 1;
                    friendRoom = j + 1;
                }
            }
    ...
    if (!can) return     cout << "Forget about apartments. Live in the dormitory.", 0;

    if (alone){
        cout << "You should rent the apartment #" << alone << " alone." << endl;
    }else cout << "You should rent the apartment #" << friendRoom << " with the friend #" << friendNum << "." << endl;
