...
int main() {
    ...
    vpll ans;
    vector<node> v(n);
    f(i, 0, n) cin >> v[i].x >> v[i].y >> v[i].r;

    f(i, 0, n){
        f(j, i + 1, n){
            ld first = abs(v[i].x - v[j].x);
            ld second = abs(v[i].y - v[j].y);
            ld dist = sqrt(first * first  + second * second);
            if (dist <= v[i].r + v[j].r) ans.push_back({i, j});
        }
    }
