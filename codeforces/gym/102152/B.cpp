    ...
    while (t--){
        ...
        f(i, 1, m + 1){
            if (memory[i] == 0){
                ...
                if (cur) v.push_back(p), cur = false;
            }
        }
        if (cur) p = {p.first, m}, v.push_back(p);
        ...
        for (auto e: v) st.insert({abs(e.first - e.second) + 1, {e.first, e.second}});
        //for (auto e: st) cout << e.first << " " << e.second.first << " " << e.second.second << endl;
        f(i, 0, q){
            cin >> a;
            auto it = st.lower_bound({a, {0, 0}});
            if (it == st.end()){
                cout << "-1 -1" << endl;
                continue;
            }
            ...
            while (it != st.end()){
                pair<int, pii> cur = *it;
                v.push_back({cur.second.second, cur.second.first}); //to sort by r then l
                it++;
            }int left, right;
            sort(rall(v));
            left = v[0].second, right = v[0].first;
            if (right - left + 1 > a){
                left = right - a + 1;
            }
