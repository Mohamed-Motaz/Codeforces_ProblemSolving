...
int main() {
    ...
    while (t--){
        ...
        string first = fun(s1, s2, s3);
        string second  = fun(s2, s1, s3);
        string third = fun(s3, s2, s1);
        string fourth = fun(s1, s3, s2);
        string fifth  = fun(s2, s3, s3);
        string sixth = fun(s3, s1, s1);

        //debug3(first, second, third);

        int fCtr = 0, sCtr = 0, tCtr = 0, fourCtr = 0, fiveCtr = 0, sixCtr = 0;
        for (auto e: first) fCtr += e == '1';
        for (auto e: second) sCtr += e == '1';
        for (auto e: third) tCtr += e == '1';
        for (auto e: fourth) fourCtr += e == '1';
        for (auto e: fifth) fiveCtr += e == '1';
        for (auto e: sixth) sixCtr += e == '1';
        if (fCtr == max({fCtr, sCtr, tCtr, fourCtr, fiveCtr, sixCtr})) cout << first;
        else if (sCtr == max({fCtr, sCtr, tCtr, fourCtr, fiveCtr, sixCtr})) cout << second;
        else if (tCtr == max({fCtr, sCtr, tCtr, fourCtr, fiveCtr, sixCtr})) cout << third;
        else if (fourCtr == max({fCtr, sCtr, tCtr, fourCtr, fiveCtr, sixCtr})) cout << fourth;
        else if (fiveCtr == max({fCtr, sCtr, tCtr, fourCtr, fiveCtr, sixCtr})) cout << fifth;
        else if (sixCtr == max({fCtr, sCtr, tCtr, fourCtr, fiveCtr, sixCtr})) cout << sixth;
        cout << endl;
    }
}