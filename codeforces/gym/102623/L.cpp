...
int main() {
    ...
    //for (auto e: v) cout << e.mx << " " << e.sum << " " << e.num << endl; cout << endl;
    while (t--){
        int arr[12];
        int ctr = 0;
        f(i, 0, 10) cin >> arr[i], arr[i] ? ctr++: ctr;
        ...
        for (auto e: v){
            ...
            int first = e.num[0] - '0';
            int second = e.num[1] - '0';
            //00 44 88
            if (first == second){
                if (arr[first] > 1){  //elements present
                    found = true;
                    ans = e.num;
                    arr[first] -= 2;
                    break;
