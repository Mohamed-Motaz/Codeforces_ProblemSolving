...
int bfs(int node){
    ...
    while (q.size()){
        ...
        for (auto e: adjList[cur.first]){
            if (!vis[e]) q.push({e, cur.second + 1});
        }
    }
    return 0;
}
int main() {
    ...
    while (t--){
        ...
        mxCtr = 0;
        dfs2(node1, 1);
        //debug4(node1, node2, mxDist, mxCtr);
        f(i,1,n + 1) vis[i] = 0;
        cout << bfs(node1) << endl;
    }
