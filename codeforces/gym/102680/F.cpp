...
ll t;
ll n, k, m, a, b, r, d, x, y, q, z, f, l, p, u;


int main() {
    MohamedMotaz;
    cin >> n >> u;
    if (u == 0) return cout << n, 0;
    vpll v(u);
    f(i, 0, u) cin >> v[i].first >> v[i].second;
    ll ans = n;
    sort(all(v));
    if (v[0].first > 1) ans = 1;
    f(i, 0, v.size() - 1){
        if (abs(v[i].second - v[i + 1].first) > 1) ans = v[i].second + 1;
    }
