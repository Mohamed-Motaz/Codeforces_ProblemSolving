...
int solve(int prev, int cur){
    if (cur == n + 1) return 0;
    int &ans = dp1[prev][cur];
    if (ans != -1) return ans;
    ans = solve(prev, cur + 1);  //leave
    if (v[cur] > v[prev]) ans = max(ans, solve(cur, cur + 1) + 1);
    return ans;
}

int solve2(int prev, int cur, int ctr){
    if (cur == n + 1) return ctr == sz;
    int &ans = dp2[prev][cur];
    if (ans != -1) return ans;
    ans = 0;
    if (dp1[prev][cur] == solve(prev, cur + 1)) ans += solve2(prev, cur + 1, ctr), ans %= mod;
    if (v[cur] > v[prev] && dp1[prev][cur] == solve(cur, cur + 1) + 1) ans += solve2(cur, cur + 1, ctr + 1), ans %= mod;
    return ans;
}
