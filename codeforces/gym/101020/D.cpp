...
#define readwrite freopen("window.in", "r", stdin);
...
int main() {
    ...
    while (t--){
        ...
        f(i, 0, n){
            ...
            int currentPos = i;
            while (st.size()){
                auto it = (lower_bound(all(st), currentPos + 1));
                if (it == st.end()) break; //element not found

                //found
                currentPos = *it;
                visited[currentPos] = 1;
                st = (mp[v[currentPos] + 1]);
                mx++;
            }
            ctr = max(ctr, mx);
            mx = 0;
