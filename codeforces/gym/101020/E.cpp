...
int main() {
    ...
    f(i, 0, t){
        int f, s, ff, ss;
        cin >> f >> s >> ff >> ss; //initial and target coordinates
        //need to check if on the same diagonal
        bool sameParity1 = f % 2 == s % 2;
        bool sameParity2 = ff % 2 == ss % 2;
        if (sameParity1 != sameParity2){
            cout << "can't reach!" << endl;
            continue;
        }
        cout << max({abs(ff - f), abs(ss - s)}) << endl;
    }
