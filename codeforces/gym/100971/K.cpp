...
int main() {
    ...
    while (ptr1 < ptr2){
        if (s[ptr1] != s[ptr2]){
            mistakes++;
            //evertree     v and e
            if (s[ptr1 + 1] == s[ptr2]) ptr1++, sol.push_back(ptr1);
                //eertteve     e and v
            else if (s[ptr2 - 1] == s[ptr1]) sol.push_back(ptr2 + 1), ptr2--;
            else return cout << "NO", 0;
        }else ptr1++, ptr2--;
    }
    if (mistakes > 1) return cout << "NO", 0;
    if (sol.size() == 0){
        return cout << "YES\n" << s.size() / 2 + 1,0;
    }else if (sol.size() == 1) return cout << "YES\n" << sol[0], 0;
    else cout << "NO";
}